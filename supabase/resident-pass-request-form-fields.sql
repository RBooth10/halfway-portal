...
-- Self-contained v2 payload function for resident portal pass request submission.
-- This replaces the prior wrapper dependency on the expanded submit_client_portal_pass_request signature.
...
declare
  link_record public.resident_portal_links%rowtype;
  resident_record public.residents%rowtype;
  clean_destination text := trim(coalesce(p_payload ->> 'destination', ''));
  clean_destination_address text := trim(coalesce(p_payload ->> 'destination_address', ''));
  clean_reason text := nullif(trim(coalesce(p_payload ->> 'reason', '')), '');
  clean_transportation text := nullif(trim(coalesce(p_payload ->> 'transportation_plan', '')), '');
  clean_emergency_plan text := nullif(trim(coalesce(p_payload ->> 'emergency_contact_plan', '')), '');
  clean_emergency_contact_name text := trim(coalesce(p_payload ->> 'emergency_contact_name', ''));
  clean_emergency_contact_relationship text := trim(coalesce(p_payload ->> 'emergency_contact_relationship', ''));
  clean_emergency_contact_phone text := trim(coalesce(p_payload ->> 'emergency_contact_phone', ''));
  clean_signature_name text := trim(coalesce(p_payload ->> 'resident_signature_name', ''));
  requested_departure timestamptz := nullif(p_payload ->> 'requested_departure_at', '')::timestamptz;
  requested_return timestamptz := nullif(p_payload ->> 'requested_return_at', '')::timestamptz;
  request_id uuid;
begin
  select *
  into link_record
  from public.resident_portal_links
  where access_token = p_payload ->> 'access_token'
    and access_token is not null
    and status = 'active'
    and (expires_at is null or expires_at > now())
  limit 1;
  ...
  if requested_departure is null or requested_return is null then
    return jsonb_build_object('ok', false, 'message', 'Departure and return times are required.');
  end if;

  if requested_return <= requested_departure then
    return jsonb_build_object('ok', false, 'message', 'Return time must be after departure time.');
  end if;
  ...
  if coalesce((p_payload ->> 'resident_agreed_to_terms')::boolean, false) is not true then
    return jsonb_build_object('ok', false, 'message', 'Resident agreement must be confirmed before submitting.');
  end if;
