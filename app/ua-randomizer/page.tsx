...
export default function UaRandomizerPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      .then(async () => {
        ...
        const loadedWindowDays = Number(savedRule?.window_days ?? windowDays);
        const loadedMinTests = Number(savedRule?.min_tests_per_window ?? minTests);
        const loadedMaxTests = Number(savedRule?.max_tests_per_window ?? maxTests);

        const autoCoverageResult = await supabase.rpc("ensure_provider_rolling_ua_schedule", {
          p_provider_id: activeProviderId,
          p_window_days: loadedWindowDays,
          p_min_tests: loadedMinTests,
          p_max_tests: loadedMaxTests,
        });

        if (autoCoverageResult.error) {
          throw autoCoverageResult.error;
        }

        const autoGeneratedCount = Number(autoCoverageResult.data ?? 0);
        ...
        if (savedRule) {
          setWindowDays(loadedWindowDays);
          setMinTests(loadedMinTests);
          setMaxTests(loadedMaxTests);
        }

        if (autoGeneratedCount > 0) {
          setMessage(`Rolling UA coverage auto-checked. ${autoGeneratedCount} missing scheduled item(s) added.`);
        }
      })
  ...
  async function syncRollingSchedule() {
    ...
    try {
      ...
      setMessage(`Rolling UA coverage checked. ${Number(data ?? 0)} missing scheduled item(s) added. Existing scheduled and completed UA records were not changed.`);
    } catch (err) {
      const syncError = err as { message?: unknown };
      setError(syncError?.message ? String(syncError.message) : "Could not check rolling UA coverage.");
    } finally {
      setSyncing(false);
    }
  }

  async function forceRepopulateRollingSchedule() {
    if (!providerId) {
      setError("No provider selected.");
      return;
    }
    ...
    try {
      ...
      setMessage(`Rolling UA schedule force-repopulated. ${Number(data ?? 0)} scheduled item(s) generated. Completed UA records were not changed.`);
    } catch (err) {
      const syncError = err as { message?: unknown };
      setError(syncError?.message ? String(syncError.message) : "Could not force repopulate rolling UA schedule.");
    } finally {
      setSyncing(false);
    }
  }
      ...
      <section className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        <div className="rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Set the rolling UA rule. The system auto-checks future coverage and adds missing scheduled items without changing completed UA records.
          </p>
          ...
          <div className="mt-5 grid gap-4">
            ...
            >
              ...
              {syncing ? "Checking..." : "Save & Check Coverage"}
            </button>

            <button
              type="button"
              onClick={() => void forceRepopulateRollingSchedule()}
              disabled={loading || syncing}
              className="inline-flex items-center justify-center gap-2 rounded-xl border bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
              <RefreshCw className="h-4 w-4" />
              Force Repopulate Backup
            </button>
          </div>
        </div>
          ...
          <div className="mt-5 space-y-3">
            {visibleScheduleRows.length === 0 ? (
              <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">
                No UA items are currently scheduled. The system will try to auto-check coverage; use Save & Check Coverage if this does not update.
              </p>
            ) : (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              The system automatically checks rolling UA coverage when this page loads and when resident status, phase,
              house assignment, admission/readmission, or discharge changes are saved. Use Force Repopulate only as a backup
              when you intentionally need to cancel and rebuild future scheduled UA items. Completed UA records are not changed.
            </p>
          </div>
        </div>
      </section>
